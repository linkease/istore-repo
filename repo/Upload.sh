...
for arch in `ls bin/packages | grep -v '^\.' | grep -v '^all$'`; do
    rsync --exclude='Packages' --exclude='Packages.*' -a bin/packages/$arch $REPO_DEST/
    rsync -a --del bin/packages/$arch/nas $REPO_DEST/$arch/
done
rsync --exclude='Packages' --exclude='Packages.*' -a bin/packages/all/nas_luci $REPO_DEST/all/
