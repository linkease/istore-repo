...
3. 如果是非开源软件请在PR信息中提供官方网站链接，并且提供软件包或二进制可执行文件的官方下载链接
4. 所有软件包文件名必须带版本号，并且每次上传都不能重名。以免缓存的版本跟索引的 Hash 值不匹配导致安装失败
5. ipk放在bin/packages下，apk放在bin/apks下，对应架构的子文件夹
6. 如果一个软件同时支持opkg和apk，那么ipk和apk都上传
7. 软件包只收集aarch64和x86_64架构，其中aarch64的ipk是aarch64_cortex-a53架构，apk是aarch64_generic架构
...
5. 在新分支中进行更改，上传ipk/apk文件。如果是更新一个包，不需要删除老版本文件，只需上传新的文件，合并时会自动清理老版本文件
